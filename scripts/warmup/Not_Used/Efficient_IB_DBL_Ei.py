...
from scipy.integrate import quad, trapezoid, IntegrationWarning  # <-- Add IntegrationWarning
...
def optimize_wrapper(args):
    E_i_val, int_max, T_cell_val, T_sun_val, fX = args
    E_i = E_i_val * eV
    T_cell = T_cell_val * u.kelvin
    T_sun = T_sun_val * u.kelvin
    return Optimize(E_i, int_max, T_cell, T_sun, fX)
...
def Optimize(E_i, int_max, T_cell, T_sun, fX):
    ...
    def constrained_efficiency(x):
        if x[0] <= x[1]:  # E_g > u_cv
            return 1e10  # Penalty for invalid
        return efficiency(x, E_i, T_cell, T_sun, int_max, fX, P_in)
    
    # Improved bounds based on analysis
    # E_g: 1.5-3.0 eV, u_cv: 0.6-0.85 of E_g
    bounds = [(1.5, 3.0), (0.6, 2.55)]  

    result = differential_evolution(constrained_efficiency,
        bounds=bounds,
        maxiter=1000,
        popsize=20,
        tol=1e-2)
    ...
    if result.success:
        print(f"E_g={result.x[0]:.3f} eV, E_i={E_i.m_as('eV'):.3f} eV, u_cv={result.x[1]:.3f} eV, Efficiency={-result.fun*100:.2f}%")
...
def main(E_i, cplx, T_cell, T_sun, int_max, fX, Plot):
    args_list = [(E_i[i].m_as('eV'), int_max, T_cell.m_as('K'), T_sun.m_as('K'), fX) for i in range(cplx)]  # <-- Convert to floats

    efficiencies = np.zeros(cplx)
    E_g = np.zeros(cplx)

    with ProcessPoolExecutor() as executor:
        results = list(tqdm(executor.map(optimize_wrapper, args_list), total=cplx))

    for i, (eff, E_g_val) in enumerate(results):
        efficiencies[i] = eff
        E_g[i] = E_g_val
...
def plot_values(E_g, E_i, efficiencies):
    ...
    plt.plot(E_i, efficiencies, marker='o', linestyle='-')
...
cplx = 15 # complexity of the calculations, higher values increase precision but also computation time

int_max = 10 # designate maximum energy to integrate to

E_i = np.linspace(0.45, 2.1, cplx) * eV  # Focus on 0.45-2.1 eV for E_i
