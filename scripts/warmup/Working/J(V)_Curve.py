...
def Optimize(E_g, int_max, T_cell, T_sun, fX):
    ...
    rough_bounds = [
        (0, E_g_val),   # E_i bounds
        (0, E_g_val),   # u_cv bounds
        (0, E_g_val),   # u_ci bounds
    ]
...
if __name__ == '__main__':
    ...
    E_g = 2.0 * eV
    E_i = 0.7 * eV
    u_cv = np.linspace(0.71, 1.99, cplx)
    ...
    E_c = E_g - E_i # type: ignore

    for ucv in u_cv:
        u_ci, valid = find_u_ci(E_g, E_c, E_i, T_cell, T_sun, fX1, ucv * eV, int_max)

        # calculate output power and efficiency
        J1.append(CB_current_density(E_g, E_c, E_i, int_max, T_cell, T_sun, fX1, ucv * eV, u_ci).m_as('mA/cm^2'))
        print(J1[-1])

    fX2 = f * X2

    for ucv in u_cv:
        u_ci, valid = find_u_ci(E_g, E_c, E_i, T_cell, T_sun, fX2, ucv * eV, int_max)

        # calculate output power and efficiency
        J2.append(CB_current_density(E_g, E_c, E_i, int_max, T_cell, T_sun, fX2, ucv * eV, u_ci).m_as('mA/cm^2'))

    # Create a single figure with both results
    plt.figure(figsize=(12, 8))
    
    # Plot curves
    plt.plot(u_cv, J1, label='X=1', color='blue')
    plt.plot(u_cv, J2, label='X=46200', color='red')

    plt.xlabel('Chemical Potential u_cv')
    plt.ylabel('Current density (A/m²)')
    plt.yscale('log')
    plt.title('Current Density vs chemical potential (${u}_{cv}$) for Different Concentration Factors')
