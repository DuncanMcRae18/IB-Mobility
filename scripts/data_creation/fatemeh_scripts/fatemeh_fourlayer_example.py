...
from fatemeh_fourlayer import multiplex_setup, run  #uses local fourlayer
...
V_exts = list(np.linspace(0, 2.25, 75+1))  # 75
...
# fmt: off
params = {
    "experiment_folder" : Path("/home/duncan/data/main/data/raw/testing_Fatemeh_script/Eg_1.67/sigma=1e-12"), # Relative path. Where to put output files. Will be created.
    "text"              : "Duncan's run of Fatemeh's script to test, Eg=1.67, Sigma_CI=Sigma_IV=1e-12, GCM=0, mu=0.001, Step size = 1e-30",     # Descriptive text, for this set of parameters
    "simple_mobility_model": False,  # True for constant IB mobility. False for fill-fraction dependent mobility
    "mu_I"              : 0.001,      # IB mobility0 cm^2/Vs
    "X"                 : 1000,      # Solar concentration factor
    "sigma_opt_ci"      : 1e-12,    # optical cross section for CI process, cm^2
    "sigma_opt_iv"      : 1e-12,
    ...
    "vth_VB"            : 2e7,
    "NI"                : 1e17,   # IB DOS cm^-3
    "IB_E"              : 0.9327205938861218, # IB energy, eV GCM=0
    "CB_E"              : 2.5,      # CB edge energy, eV (bandgap)
    "VB_E"              : 0.0,
...
simudoparams = {
    "mesh_start"        : 0.001,    # smallest mesh segment, in um
    ...
    "opt_success_factor": 2.0,        # Factor for increasing step size on successful Newton iteration in optical stepper (3)
    "opt_fail_factor"   : 0.2,      # Factor for decreasing step size on failed Newton iteration in optical stepper (0.2)
    "V_success_factor"  : 1.5,      # Factor for increasing step size on successful Newton iteration in V stepper (1.5)
    "V_fail_factor"     : 0.5,      # Factor for decreasing step size on failed Newton iteration in V stepper (0.5)
    "find_unconverged"  : False     # Extra output in logs at failed Newton steps about which parameters failed to converge
}
...
analysisparams = {
    ...
    "optimize_bounds"   : [0.1, 1],  # range for fminbnd to use on optimize_key
