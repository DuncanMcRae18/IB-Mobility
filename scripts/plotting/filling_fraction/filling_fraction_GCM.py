...
plot_folder = '/home/duncan/data/repo/plots/filling_fraction/GCM'
...
def main():
    ...
    # Group: sims[Eg/GCM][ci/iv][mu_I] = (voltage_file, sim_dir)
    ...
    for voltage_file in pd_v_files:
        ...
        if len(parts) < 6:
            continue
        eg, mu_I, ci, iv, gcm = parts[0], parts[1], parts[2], parts[3], parts[4]
        plot_key = f'{eg}/{gcm}'
        ci_iv_key = f'{ci}/{iv}'
        
        sims[plot_key][ci_iv_key][mu_I] = (voltage_file, sim_dir)
    ...
    for plot_key in sorted(sims.keys()):
        ...
        for idx, ci_iv in enumerate(ci_iv_combos):
            ax = axes[idx // 2, idx % 2]
            mu_I_data = ci_iv_groups.get(ci_iv, {})
            
            for mu_I in mu_I_values:
                if mu_I not in mu_I_data:
                    continue
                
                voltage_file, sim_dir = mu_I_data[mu_I]
            ...
            ax.set_title(f'{ci_iv.replace("/", ", ")}', fontsize=11)
        ...
        output_path = output_dir / 'filling_fraction_by_mu_I.png'
