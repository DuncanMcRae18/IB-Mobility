...
base_plot_path = "/home/duncan/data/repo/plots/efficiency"
...
# Data Cleaning
numeric_cols = ['CB_E', 'eff', 'mu_I', 'sigma_opt_ci', 'sigma_opt_iv']
...
def main():
    ...
    for Eg in CB_E_vals:
        # --- TASK 1: THE GRID PLOT ---
        fig_grid, axes = plt.subplots(2, 2, figsize=(14, 10), sharex=True, sharey=True)
        fig_grid.suptitle(f'Efficiency vs $\mu_I$ Grid ($E_g$ = {Eg} eV)', fontsize=18)
        ...
        for idx, (ci, iv) in enumerate(sigma_combinations):
            ...
            # --- TASK 2: THE INDIVIDUAL PLOT ---
            fig_ind, ax_ind = plt.subplots(figsize=(10, 6))
            
            for GCM in GCM_vals:
                subset_gcm = subset[subset['GCM'] == GCM].sort_values('mu_I')
                if not subset_gcm.empty:
                    # Plot on both the grid and the individual figure
                    line_params = {'marker': 'o', 'linestyle': '-', 'label': f'$m_G$={GCM}'}
                    ax.semilogx(subset_gcm['mu_I'], subset_gcm['eff'], markersize=4, **line_params)
                    ax_ind.semilogx(subset_gcm['mu_I'], subset_gcm['eff'], **line_params)
            ...
            ax_ind.set_ylabel('Efficiency', fontsize=font)
            ax_ind.grid(True, alpha=0.3); ax_ind.legend(fontsize=font-2)
            
            ci_s = f'{ci:.1e}'.replace('-', 'm'); iv_s = f'{iv:.1e}'.replace('-', 'm')
            fig_ind.savefig(os.path.join(indiv_path, f'eff_Eg{Eg}_ci{ci_s}_iv{iv_s}.png'), dpi=150)
            plt.close(fig_ind)

            # Finalize Subplot in Grid
        ...
        fig_grid.text(0.02, 0.5, 'Efficiency', va='center', rotation='vertical', fontsize=16)
        fig_grid.tight_layout(rect=[0.05, 0.05, 1, 0.95])
        fig_grid.savefig(os.path.join(grid_path, f'eff_grid_Eg{Eg}.png'), dpi=200)
        plt.close(fig_grid)
        print(f"Completed Efficiency suite for Eg={Eg}")
