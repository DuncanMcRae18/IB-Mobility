...
plot_folder = '/home/duncan/data/repo/plots/NR_recombination/individual'
...
def plot(spatial001, spatial30):
    ...
    plt.semilogy(
        ...
        np.array(spatial001["g_nr_top_IB"]),
        color="green",
        label=r"$g_{iv}^{0.001}$",
    )
    plt.semilogy(
        np.array(spatial001["coord_x"]),
        -np.array(spatial001["g_nr_bottom_IB"]),
        color="red",
        label=r"$g_{ci}^{0.001}$",
    )
...
def main():
    ...
    ax.set_ylabel(r'NR Recombination $(\mathrm{cm}^{-3}\mathrm{s}^{-1})$', fontsize=font)
    ax.legend(fontsize=font)
    ax.grid(True, alpha=0.3)
    fig.tight_layout()

    output_path = Path(plot_folder) / 'nr_recomb_individual.png'
