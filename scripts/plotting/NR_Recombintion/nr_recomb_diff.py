...
def plot(spatial001, spatial30):
    ...
    CI_diff = np.array(spatial30["g_nr_top_IB"]) - np.array(spatial001["g_nr_top_IB"])
    IV_diff = np.array(spatial30["g_nr_bottom_IB"]) - np.array(spatial001["g_nr_bottom_IB"])
...
def main():
    ...
    output_path = Path(plot_folder) / 'nr_recomb_diff.png'
