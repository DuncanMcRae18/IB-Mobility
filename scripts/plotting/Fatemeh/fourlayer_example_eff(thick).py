...
V_exts = list(np.linspace(0, 1.5, 75+1))  # For more precision, use 75
...
# fmt: off
params = {
    "experiment_folder" : Path("AUG19c/"), # Relative path. Where to put output files. Will be created.
    "text"              : "Getting effciency as a function of IB thickness for mu_I = 100, GCM=10, sigma=1e-13",     # Descriptive text, for this set of parameters
    "simple_mobility_model": False,  # True for constant IB mobility. False for fill-fraction dependent mobility
    "mu_I"              : 100,      # IB mobility0 cm^2/Vs
    "X"                 : 1000,      # Solar concentration factor
    "sigma_opt_ci"      : 1e-13,    # optical cross section for CI process, cm^2
    "sigma_opt_iv"      : 1e-13,
    ...
    "vth_VB"            : 2e7,
    "NI"                : 1e17,   # IB DOS cm^-3
    "IB_E"              : 0.5544717324472143,      # IB energy, eV
    "CB_E"              : 1.67,      # CB edge energy, eV (bandgap)
    "VB_E"              : 0.0,
    ...
    "prefix"            : "fourlayer",
    "multiplex_keys"    : ["IB_thickness"],     # Which keys to use for multiplexing, eg ["NI"]
    "title_keys"        : ["NI","mu_I","X","sigma_opt_ci"], # Make an informative title for plots etc
}

thicks = list(np.arange(0.1, 2.1, 0.1))
...
analysisparams = {
    ...
    "optimize_bounds"   : [0.1, 2],  # range for fminbnd to use on optimize_key
    "optimize_xtol"     : 1e-2,      # for fminbound to find efficiency
    "multiprocess"      : True,     # Use multiprocessing
    "multiprocess_pool" : 2,         # Number of CPU"s to use for multiprocess
...
def logging_shutdown(): #Added this to see if it fixes the log file issue
    log = logging.getLogger()
    #Need to remove the handlers twice to get them really removed.
