...
::
    ...
    def jv(self,extra_fields=None):
        ...
        for fname in files:
            ...
            contact_name = self.contact_prefix[0] + "_contact"
            j = (
                data["avg:current_CB:" + contact_name]["value"]
                + data["avg:current_VB:" + contact_name]["value"]
            )
            cur_dict = {"j": j, "v": v, "p": j * v, "file": fname}            
    ...
    def get_spatial_data(self, row, num_points = 5001):
        ...
        if path.isfile(spatial_file):
            ...
            for f in spatialx.func_names["funcs"]:
                d[f] = np.array(spatialx.line_cut(getattr(spatialx,f),   num_points)[0])
            d["coord_x"]=np.array(spatialx.line_cut(getattr(spatialx,f), num_points)[1])
            return d
...
def IB_band_diagram(spatial, IB_mask = True):
    ...
    if "Ephi_IB" in spatial.dtype.names:
        plt.plot(
    ...
    if "qfl_IB" in spatial.dtype.names:
        plt.plot(
